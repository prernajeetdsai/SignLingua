...
.s-warn { background:#1a1400; border:1px solid #3a3000; border-radius:4px;
    padding:.5rem 1rem; font-family:'DM Mono',monospace; font-size:.62rem;
    color:#b8960a; letter-spacing:.1em; margin-bottom:.9rem; }
...
def mic_section(label: str, key: str, hint: str = "") -> object:
    """Live mic recorder using st.audio_input (Streamlit >= 1.33)."""
    st.markdown(f'<div class="mic-label">{label}</div>', unsafe_allow_html=True)
    # Must pass non-empty label; hide it visually with label_visibility="hidden"
...
with st.sidebar:
    ...
    hr()
    st.markdown("""<div class="tip"><p>
        <strong>STT Engine:</strong> OpenAI Whisper (local)<br>
        No internet needed for transcription.<br>
        No API key. No rate limits.<br><br>
        OCR text from <strong>Tab 1</strong> is auto-indexed
        so you can ask questions about it in Tab 3.
    </p></div>""", unsafe_allow_html=True)
...
<div class="masthead">
  <div>
    ...
    <div class="mh-sub">OCR · Live Voice Translation · Gemini 2.5 Flash AI · Whisper STT · FAISS RAG</div>
  </div>
  <div class="mh-meta">
    40+ Languages &nbsp;·&nbsp; Tesseract OCR<br>
    Whisper STT (local) &nbsp;·&nbsp; Gemini 2.5 Flash<br>
    FAISS RAG &nbsp;·&nbsp; gTTS · Live Mic
  </div>
</div>
...
# ╚═══════════════════════════════════════════════════════╝
with tab2:
    pipe_bar(["🎙 Record Voice", "Whisper STT", "Detect Language",
    ...
    with L2:
        voice_rec2 = mic_section(
            ...
            hint=(
                ...
                "Click again to stop. Then press <strong>Translate Recording</strong>.<br>"
        ...
        st.markdown("""<div class="tip"><p>
            <strong>STT Engine:</strong> OpenAI Whisper (base model, runs locally).<br>
            Supports 99 languages, auto-detects language spoken.<br>
            First run downloads the model (~74 MB) — cached afterwards.
        </p></div>""", unsafe_allow_html=True)
    ...
    with R2:
        ...
        if run2:
            if not voice_rec2:
                ...
                with st.spinner("Transcribing with Whisper… (first run downloads model ~74MB)"):
                    try:
...
# ╚═══════════════════════════════════════════════════════╝
with tab3:
    pipe_bar(["🎙 Record Question", "Whisper STT", "RAG Retrieval",
    ...
    with L3:
        voice_rec3 = mic_section(
            ...
            key="mic_tab3",
            hint=(
                "Press mic to record · Press again to stop.<br>"
                "Then click <strong>Ask Gemini</strong>.<br>"
                "Whisper transcribes locally, Gemini answers online."
            )
        )
        ...
        st.markdown("""<div class="tip"><p>
            ...
            · Whisper STT (local transcription)<br>
            · FAISS RAG context from indexed signs / docs<br>
            · Full conversation memory (multi-turn)<br>
    ...
    with R3:
        ...
        if run3:
            if not gemini_key:
                ...
                with st.spinner("Transcribing with Whisper · Querying Gemini…"):
                    try:
                        ...
                        if "Gemini" in str(e) or "API" in str(e):
                            st.info("Check your Gemini API key at aistudio.google.com → API Keys. Make sure billing is enabled if on paid tier.")
                    except Exception as e:
                        st.error(f"Unexpected: {e}")
...
<div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:.8rem;">
  <div style="font-family:'DM Mono',monospace;font-size:.57rem;color:#24202a;letter-spacing:.1em;">
    SignLingua &nbsp;·&nbsp; OCR · Live Voice · Gemini AI
  </div>
