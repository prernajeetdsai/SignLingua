...
with st.sidebar:
    ...
    st.markdown("""<div class="tip" style="margin-bottom:.8rem;margin-top:0;"><p>
        <strong>Supported formats:</strong><br>
        PDF · Word (docx) · PowerPoint (pptx)<br>
        Excel (xlsx) · CSV · Text · Markdown
    </p></div>""", unsafe_allow_html=True)

    kb_doc = st.file_uploader(
        "Upload document to knowledge base",
        type=SUPPORTED_KB_EXTENSIONS,
        key="kb_doc",
        label_visibility="collapsed",
    )
    if kb_doc:
        ext = kb_doc.name.rsplit(".", 1)[-1].upper()
        st.markdown(
            f'<div class="lbadge" style="margin-bottom:.6rem;">'
            f'<span class="ldot"></span>{ext} &nbsp;·&nbsp; {kb_doc.size/1024:.1f} KB</div>',
            unsafe_allow_html=True)
        if st.button("Index Document", key="idx_doc"):
            with st.spinner(f"Extracting & embedding {kb_doc.name}…"):
                try:
                    file_bytes = kb_doc.read()
                    text = extract_text_from_file(file_bytes, kb_doc.name)
                    added = add_text_to_store(text, source=kb_doc.name)
                    st.success(f"✓ Indexed {added} chunks from {kb_doc.name}")
                    st.rerun()
